...
import fr.javafx.scene.control.chart.plugins.OverlayChartPlugin;
import fr.javafx.scene.control.chart.plugins.OverlayChartPlugin.OverlayArea;
import fr.javafx.scene.control.chart.plugins.behavior.XYChartPanner;
import fr.javafx.scene.control.chart.plugins.behavior.XYChartZoomer;
...
class DataIndicatorsSample extends AbstractSamplePane {
	...
	protected Node createSamplePane() {
		...
		chartPane.getPlugins().add(new OverlayChartPlugin<>(OverlayArea.PLOT_AREA, anchorPane));
		chartPane.getPlugins().add(xValueIndicator);
		chartPane.getPlugins().add(yRangeIndicator);
		chartPane.getPlugins().add(new XYChartZoomer());
        chartPane.getPlugins().add(new XYChartPanner());
	...
	private static List<String> styles(String... cssNames) {
		Path path = Paths.get("/ssd/src/Covid-19/github/covid-19/src/main/resources/cern/tests");
		return Arrays.stream(cssNames)
					 .map(css -> path.resolve(css).toString())
					 .collect(Collectors.toList());
//		return Arrays.stream(cssNames)
//					 .map(css -> { System.out.println(css);return DataIndicatorsSample.class.getResource("/tests/" + css).toExternalForm(); })
//					 .collect(Collectors.toList());
	}
