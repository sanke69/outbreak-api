...
public class XYChartZoomer extends AbstractChartPlugin<Number, Number> {
    ...
    public XYChartZoomer(XY.Constraint zoomMode) {
        this(zoomMode, false);
    }

    public XYChartZoomer(boolean animated) {
        this(XY.Constraint.BOTH, animated);
    }
    public XYChartZoomer(XY.Constraint zoomMode, boolean animated) {
    ...
    private void 										clearZoomStackIfAxisAutoRangingIsEnabled() {
        for (XYChart<Number, Number> chart : getCharts()) {
            if ((getAxisMode().allowsHor() && chart.getXAxis().isAutoRanging())
                    || (getAxisMode().allowsVer() && chart.getYAxis().isAutoRanging())) {
                clear();
                return;
            }
        }
