...
import fr.javafx.scene.control.chart.XY.Mode;
...
public class XYChartPanner extends AbstractChartPlugin<Number, Number> {
    ...
    private final ObjectProperty<Mode> axisMode = new SimpleObjectProperty<Mode>(this, "axisMode", Mode.XY) {
        @Override
    ...
    public XYChartPanner(XY.Mode panMode) {
        setAxisMode(panMode);
     ...
     */
    public final ObjectProperty<Mode> axisModeProperty() {
        return axisMode;
    }
     ...
     */
    public final void setAxisMode(Mode mode) {
    ...
    private void panChart(XYChart<Number, Number> chart, Point2D mouseLocation) {
        ...
        if (!XYChartUtils.Axes.hasBoundedRange(xAxis) && getAxisMode().allowsX()) {
            xAxis.setAutoRanging(false);
            shiftBounds(xAxis, xOffset);
        }
        ValueAxis<?> yAxis = XYChartUtils.Axes.toValueAxis(chart.getYAxis());
        if (!XYChartUtils.Axes.hasBoundedRange(yAxis) && getAxisMode().allowsY()) {
