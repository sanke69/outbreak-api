...
import static fr.javafx.utils.FxUtils.*;
...
//		double height = yAxis.getDisplayPosition( yAxis.toRealValue( yAxis.getLowerBound() ) );
	...
	public XYChartInputContext getContext(double _x, double _y) {
		if(getXAxisArea().contains( _x, _y ))
			return XYChartInputContext.onXAxis;
		if(getYAxisArea().contains( _x, _y ))
			return XYChartInputContext.onYAxis;
		if(getPlotArea().contains( _x, _y ))
			return XYChartInputContext.inPlotArea;
		return XYChartInputContext.outsideChart;
	}
